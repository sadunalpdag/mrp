# ==============================================================
#  📘 EMA ULTRA v12.2 — AI Param Archive (Full System)
#  Binance Futures EMA+ATR+RSI+ADX+AI Learning botu (Render-ready)
# ==============================================================
...
def is_weekend_ist():
    # 0=Mon ... 6=Sun
    return now_ist_dt().weekday() in (5, 6)
...
def apply_model_to_params(model):
    """AI/istatistik özetine göre PARAM'a küçük adımlarla ayar uygula."""
...
def collect_training_df(days_back=14, weekend=False):
    ...
    # Feature engineering (CSV sütunları)
    ...
    # basit proxy: trend_match ve atr_pct yoksa 0 varsay
...
def maybe_daily_summary_and_learn(state):
    ...
    if now_ist_hhmm() >= DAILY_SUMMARY_TIME and state.get("last_daily_summary_date") != today_ist_date():
        ...
        # AI Learning update + param snapshot
