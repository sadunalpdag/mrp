# ==============================================================================
# 📘 EMA ULTRA v15.6-R — Real-Only Instant-Scalp (reduceOnly off)
# ==============================================================================
...
def futures_get_klines(s,i,l):
    try:
        r=requests.get(BINANCE_FAPI+"/fapi/v1/klines",
            params={"symbol":s,"interval":i,"limit":l},timeout=10).json()
...
def get_symbol_filters(s):
    try:
        ...
        pf =next((f for f in x["filters"] if f["filterType"]=="PRICE_FILTER"),{})
        return {"stepSize":float(lot.get("stepSize","1")),"tickSize":float(pf.get("tickSize","0.01"))}
    except:return {"stepSize":1.0,"tickSize":0.01}
def round_nearest(x,st): return round(round(x/st)*st,8)
...
def calc_order_qty(s,price,usd): return adjust_precision(s,usd/max(price,1e-12),"qty")
...
def open_market_position(s,dir,qty):
    ...
    res=_signed_request("POST","/fapi/v1/order",
        {"symbol":s,"side":side,"type":"MARKET","quantity":f"{qty}",
    ...
    return {"symbol":s,"dir":dir,"entry":p,"qty":qty}
...
def futures_set_tp_sl(s,dir,qty,entry,tp,sl):
    ...
    for t,pv in [("TAKE_PROFIT_MARKET",tp_s),("STOP_MARKET",sl_s)]:
        payload={"symbol":s,"side":close,"type":t,"stopPrice":f"{pv:.8f}",
...
def rsi(v,n=14):
    ...
    ag,al=np.mean(g[:n]),np.mean(l[:n]);out=[50]*n
    for i in range(n,len(d)):
        ag=(ag*(n-1)+g[i])/n;al=(al*(n-1)+l[i])/n;rs=ag/al if al>0 else 0
        out.append(100-100/(1+rs))
    return [50]*(len(v)-len(out))+out
def atr_like(h,l,c,n=14):
    tr=[]
    for i in range(len(h)):
        tr.append(max(h[i]-l[i],abs(h[i]-c[i-1]) if i else 0))
    a=[sum(tr[:n])/n]
...
def build_scalp_signal(sym,kl,bar):
    ...
    if not d or abs(s_now-s_prev)<PARAM["ANGLE_MIN"]:return None
...
def trade_ultra(sig):
    ...
    try:
        o=open_market_position(s,d,qty)
        futures_set_tp_sl(s,d,qty,o["entry"],PARAM["SCALP_TP_PCT"],PARAM["SCALP_SL_PCT"])
        tg_send(f"✅ REAL {s} {d} qty:{qty} entry:{o['entry']:.6f}")
        AI_RL.append({"time":now_local_iso(),"symbol":s,"dir":d,"entry":o["entry"],"power":sig["power"]})
...
# ---------- main ----------
def main():
    tg_send("🚀 EMA ULTRA v15.6-R başladı (Real-Only, reduceOnly off)")
